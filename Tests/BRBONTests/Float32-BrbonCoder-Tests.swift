...
class Float32_BrbonCoder_Tests: XCTestCase {
    ...
    func test_NoNameField() {
        ...
        XCTAssertEqual(Float32.readValue(atPtr: buffer.baseAddress!, machineEndianness), 1.23)
        
        buffer.copyBytes(from: exp)
        
        XCTAssertEqual(Float32.readFromItem(atPtr: buffer.baseAddress!, machineEndianness), 1.23)
        
        buffer.copyBytes(from: [0xa4, 0x70, 0x9d, 0x3f])
        
        XCTAssertEqual(Float32.readFromElement(atPtr: buffer.baseAddress!, machineEndianness), 1.23)
    }
