...
class ItemValue_Tests: XCTestCase {
    ...
    func testDictionary() {
        ...
        XCTAssertNil(itemValue.elementByteCount)
        XCTAssertEqual(itemValue.byteCount, 4)
        XCTAssertEqual(itemValue.endianBytes(.little), Data(bytes: [0x0, 0x0, 0x0, 0x00]))

        let item1 = Item(ItemValue(Int8(5)))
        itemValue.array.append(item1)
        //let item2 = Item(ItemValue(Int8(3)))
        //itemValue.array.append(item2)
        XCTAssertEqual(itemValue.endianBytes(.little), Data(bytes: [0x1, 0x0, 0x0, 0x00, 0x2, 0x0, 0x0, 0x0, 0x8, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]))

        
        var data = Data(bytes: [0x1, 0x0, 0x0, 0x00, 0x2, 0x0, 0x0, 0x0, 0x8, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
        var bytePtr = (data as NSData).bytes
        var counter = UInt32(data.count)
        ...
        if let value = ItemValue(&bytePtr, count: &counter, endianness: .little, type: .dictionary) {
            ...
            XCTAssertNil(value.elementType)
            XCTAssertNil(value.elementByteCount)
            XCTAssertEqual(value.byteCount, 20)
            XCTAssertEqual(value.endianBytes(.little), Data(bytes: [0x1, 0x0, 0x0, 0x00, 0x2, 0x0, 0x0, 0x0, 0x8, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]))
        }
    }
    ...
    func testBinary() {
        ...
        XCTAssertNil(itemValue.elementByteCount)
        XCTAssertEqual(itemValue.byteCount, 4)
        XCTAssertEqual(itemValue.endianBytes(.little), Data(bytes: [0x00, 0x0, 0x0, 0x0]))
        ...
        XCTAssertNil(itemValue.elementType)
        XCTAssertNil(itemValue.elementByteCount)
        XCTAssertEqual(itemValue.byteCount, 9)
        XCTAssertEqual(itemValue.endianBytes(.little), Data(bytes: [0x05, 0x0, 0x0, 0x0, 1, 2, 3, 4, 5]))
