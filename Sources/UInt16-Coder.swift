...
extension UInt16: Coder {
    ...
    var brbonType: ItemType { return ItemType.uint16 }
...
extension UInt16: Initialize {
    
    init(valuePtr: UnsafeMutableRawPointer, count: Int = 0, _ endianness: Endianness) {
        if endianness == machineEndianness {
            self.init(valuePtr.assumingMemoryBound(to: UInt16.self).pointee)
        } else {
            self.init(valuePtr.assumingMemoryBound(to: UInt16.self).pointee.byteSwapped)
        }
    }
