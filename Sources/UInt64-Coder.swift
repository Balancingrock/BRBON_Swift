...
extension UInt64: Coder {
    ...
    var brbonType: ItemType { return ItemType.uint64 }
...
extension UInt64: Initialize {
    
    init(valuePtr: UnsafeMutableRawPointer, count: Int = 0, _ endianness: Endianness) {
        if endianness == machineEndianness {
            self.init(valuePtr.assumingMemoryBound(to: UInt64.self).pointee)
        } else {
            self.init(valuePtr.assumingMemoryBound(to: UInt64.self).pointee.byteSwapped)
        }
    }
